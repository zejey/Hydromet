  ...
  return SingleChildScrollView(
    ...
    child: Row(
      children: hourlyForecast.take(12).map((hour) {
         // 1. Handle 'time' which is a DateTime or String, not UNIX timestamp
        DateTime time;
        if (hour['time'] is String) {
          ...
          // fallback in case time is missing or in unknown format
          time = DateTime.now();
          print("Warning: 'time' missing or invalid in hourly forecast item: $hour");
        }
        final formattedTime = TimeOfDay.fromDateTime(time).format(context);

        // 2. Use 'icon' directly
        String iconCode = hour['icon'] ?? '01d'; // fallback icon

        // 3. Temperature (round safely)
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    if (weatherData != null) ...[
                                      ...
                                      if (hourlyForecast.isNotEmpty) buildHourlyForecast(),
